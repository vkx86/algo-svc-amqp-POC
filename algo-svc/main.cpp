...
int main(int argc, char *argv[]) {
    ...
    if( !logFile.empty()){
        logFile += "ALGO_SVC_%N.log";
        initLog(logFile);
    }

    BOOST_LOG_TRIVIAL(info) << "ALGO_SVC startup...";

    auto amqp_conn_str = boost::any_cast<std::string>(vm["amqp-conn"].value());
    AmqpPub amqpPub(amqp_conn_str, {EXCH_VIDEO_OUT});

    AlgoSvcSim sim(amqpPub);

    AmqpSub amqpSub(amqp_conn_str, {{EXCH_VIDEO_IN,ROUTE_VIDEO_IN}}, "msg-sweeper-ALGO_SVC@03c348a1d8db4159875e77ac2558214e", &sim);
    ...
    BOOST_LOG_TRIVIAL(info) << "ALGO_SVC shutdown...";
    return 0;
}